...
{/if}
<input type="hidden" name="{$fields.marketing_c.name}" value="0"> 
<input type="checkbox" id="{$fields.marketing_c.name}" 
name="{$fields.marketing_c.name}" 
...
{if strlen($fields.bilancio_c.value) <= 0}
{assign var="value" value=$fields.bilancio_c.default_value }
{else}
{assign var="value" value=$fields.bilancio_c.value }
{/if}  
<input type='text' name='{$fields.bilancio_c.name}'
id='{$fields.bilancio_c.name}'
size='30'
...
{minify}
{capture name="label" assign="label"}{sugar_translate label='Tipo Marketing' module='Contacts'}{/capture}
{$label|strip_semicolon}:
...
<select name="{$fields.tipomarketing_c.name}" 
id="{$fields.tipomarketing_c.name}" 
title=''       
>
{if isset($fields.tipomarketing_c.value) && $fields.tipomarketing_c.value != ''}
{html_options options=$fields.tipomarketing_c.options selected=$fields.tipomarketing_c.value}
{else}
{html_options options=$fields.tipomarketing_c.options selected=$fields.tipomarketing_c.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.tipomarketing_c.options }
{capture name="field_val"}{$fields.tipomarketing_c.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.tipomarketing_c.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.tipomarketing_c.name}" 
id="{$fields.tipomarketing_c.name}" 
title=''          
>
{if isset($fields.tipomarketing_c.value) && $fields.tipomarketing_c.value != ''}
...
<input
id="{$fields.tipomarketing_c.name}-input"
name="{$fields.tipomarketing_c.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.tipomarketing_c.name}-image"></button><button type="button"
id="btn-clear-{$fields.tipomarketing_c.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.tipomarketing_c.name}-input', '{$fields.tipomarketing_c.name}');sync_{$fields.tipomarketing_c.name}()"><span class="suitepicon suitepicon-action-clear"></span></button>
</span>
{literal}
<script>
	...
	{/literal}
			...
			{literal}
		(function (){
			var selectElem = document.getElementById("{/literal}{$fields.tipomarketing_c.name}{literal}");
	...
	SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.tipomarketing_c.name}-input');
	SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.tipomarketing_c.name}-image');
	SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.tipomarketing_c.name}');
			...
			//global variable 
			sync_{/literal}{$fields.tipomarketing_c.name}{literal} = function(){
				SyncToHidden();
			}
			function syncFromHiddenToWidget(){
				...
				for (i=0;i<selectElem.options.length;i++){
					...
					if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.tipomarketing_c.name}-input{literal}'))
						SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
				}
			}

            YAHOO.util.Event.onAvailable("{/literal}{$fields.tipomarketing_c.name}{literal}", syncFromHiddenToWidget);
		{/literal}
...
addToValidate('EditView', 'tipomarketing_c', 'enum', false,'{/literal}{sugar_translate label='LBL_TIPOMARKETING' module='Contacts' for_js=true}{literal}' );
addToValidate('EditView', 'opportunities_contacts_1_name', 'relate', false,'{/literal}{sugar_translate label='LBL_OPPORTUNITIES_CONTACTS_1_FROM_OPPORTUNITIES_TITLE' module='Contacts' for_js=true}{literal}' );
...
addToValidate('EditView', 'marketing_c', 'bool', false,'{/literal}{sugar_translate label='LBL_MARKETING' module='Contacts' for_js=true}{literal}' );
addToValidate('EditView', 'bilancio_c', 'decimal', false,'{/literal}{sugar_translate label='LBL_BILANCIO' module='Contacts' for_js=true}{literal}' );
